<?php
     ...
     if (!isset($_GET['code'])) {
          ...
          $code = $_GET['code'];
          $selectReceipt = $getFromU->select_all_val_row('product_purchase', 'trans_code', $code);
          $customerInfo = !empty($selectReceipt?->customer_id) ? $getFromU->select_all_val_row('customers', 'id', $selectReceipt?->customer_id) : "";
...
<body>
     ...
     <div id="printReceipt">
          ...
          <?php
               $ids = explode(',', $selectReceipt->product_id);
               $qty = explode(',', $selectReceipt->quantity);
               $tt_price = explode(',', $selectReceipt->total_price);
          ?>
          
          <div class="transaction-info">
               <span>Date: <?= date('d/m/Y H:i:s', strtotime($selectReceipt->created_at)) ?></span>
          ...
          <div class="products-list">
               ...
               foreach ($ids as $key => $id):
                    $item_total = $tt_price[$key] ?? 0;
                    $subtotal += $item_total;

                    $prod_name = $getFromU->select_one_val('products', 'name', 'id', $ids[$key]);
                    // $prod_price = $getFromU->select_one_val('products', 'price', 'id', $ids[$key]);
                    $is_pack = $getFromU->select_one_val('products', 'is_pack', 'id', $ids[$key]);
                    $pack_size = $getFromU->select_one_val('products', 'pack_size', 'id', $ids[$key]);
                    if ($is_pack == 1) {
                         list($packPart, $singlePart) = explode('+', $qty[$key]);

                         $packQty   = (int) explode('|', $packPart)[0];
                         $singleQty = (int) explode('|', $singlePart)[0];

                         $quantityCalc = (($packQty * $pack_size) + $singleQty) / $pack_size;
                         $prod_price = (float) $item_total / $quantityCalc;
                    } else {
                         $prod_price = $getFromU->select_one_val('products', 'price', 'id', $ids[$key]);
                    }
               ?>
                    <div class="receipt-product">
                         <span class="product-name"><?= strtoupper(htmlspecialchars($prod_name)) ?></span>
                         <div class="product-details">
                              <span class="qty-price">
                                   <?= $qty[$key] ?> x <?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format($prod_price, 2) ?>
                              </span>
                              <span class="total">
                                   <?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format($item_total, 2) ?>
                              </span>
                         </div>
                    </div>
               <?php endforeach; ?>
          ...
          <div class="totals-section">
               <div class="total-row">
                    ...
                    <span class="total-value"><?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format($subtotal, 2) ?></span>
               </div>
               
               <?php if ($selectReceipt->vat > 0): ?>
               <div class="total-row">
                    <span class="total-label">VAT (<?= number_format($selectReceipt->vat, 2) ?>):</span>
                    <span class="total-value"><?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format($selectReceipt->vat, 2) ?></span>
               </div>
               <?php endif; ?>
               
               <div class="total-row">
                    <span class="total-label">Grand Total:</span>
                    <span class="total-value"><?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format($selectReceipt->grand_total + (float)($selectReceipt->vat ?? 0), 2) ?></span>
               </div>
          </div>
          ...
          <div class="payment-section">
               <div class="payment-row">
                    ...
                    <span><?= strtoupper($selectReceipt->payment_mode) ?></span>
               </div>
               
               <div class="payment-row">
                    <span>Amount Paid:</span>
                    <span><?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format(($selectReceipt->grand_total + (float)($selectReceipt->vat ?? 0)) - $selectReceipt->credit, 2) ?></span>
               </div>
               
               <?php if ($selectReceipt->credit > 0): ?>
               <div class="payment-row">
                    <span>Balance Due:</span>
                    <span><?= $getFromU->getCurrencySymbol($selectDefaultCurrency) ?><?= number_format($selectReceipt->credit, 2) ?></span>
               </div>
               <?php endif; ?>
          </div>

          <div class="additional_info">
               <?= $selectReceipt?->additional_info ?>
          </div>
