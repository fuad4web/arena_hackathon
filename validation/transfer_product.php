...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM products WHERE branch_id = ? AND code = ?");
        $stmt->execute([$sourceBranch, $productCode]);
        ...
        $stmt = $pdo->prepare("UPDATE products SET quantity = quantity - ? WHERE branch_id = ? AND code = ?");
        $stmt->execute([$transferQuantity, $sourceBranch, $productCode]);

        // Check if product exists in destination branch
        $stmt = $pdo->prepare("SELECT * FROM products WHERE branch_id = ? AND code = ?");
        $stmt->execute([$destinationBranch, $productCode]);
